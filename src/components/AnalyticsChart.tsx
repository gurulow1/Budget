...
// Global Data
const SECTORS = [
  { id: 'edu', name: 'Образование', value: 32, color: '#2563EB', kpis: ['🚀 +12% рост качества', '🎓 15 новых школ'] },
  { id: 'trans', name: 'Транспортная система', value: 28, color: '#8B5CF6', kpis: ['🚇 6 новых станций', '⚡ +120 электробусов'] },
  { id: 'health', name: 'Здравоохранение', value: 20, color: '#CC1111', kpis: ['🏥 8 новых поликлиник', '❤️ +15% ЭМИАС'] },
  { id: 'soc', name: 'Социальная поддержка', value: 15, color: '#10B981', kpis: ['🤝 Выплаты 2.1 млн чел.', '👴 10 новых клубов'] },
  { id: 'gkh', name: 'ЖКХ и благоустройство', value: 5, color: '#F59E0B', kpis: ['🌳 15 новых парков', '💡 Модернизация сетей'] },
];

const SUBCATEGORIES: Record<string, { name: string; value: number }[]> = {
  edu: [
    { name: 'Школы и детские сады (общее образование)', value: 55 },
    { name: 'Колледжи (среднее профобразование)', value: 25 },
    { name: 'ВУЗы, высшее образование и наука', value: 20 },
  ],
  trans: [
    { name: 'Строительство метро (Троицкая, РАЛ)', value: 60 },
    { name: 'Наземный общественный транспорт (электробусы)', value: 25 },
    { name: 'Пригородные сообщения и МЦД', value: 15 },
  ],
  health: [
    { name: 'Поликлиники, амбулаторное лечение', value: 45 },
    { name: 'Стационарная и высокотехнологичная помощь', value: 35 },
    { name: 'Развитие сервисов ЭМИАС', value: 20 },
  ],
  soc: [
    { name: 'Прямые выплаты льготным категориям', value: 65 },
    { name: 'Программа МосДолголетие и развитие клубов', value: 20 },
    { name: 'Реабилитация и социальная интеграция', value: 15 },
  ],
  gkh: [
    { name: 'Модернизация инженерных сетей', value: 40 },
    { name: 'Капитальный ремонт жилого фонда', value: 35 },
    { name: 'Благоустройство парков и скверов', value: 25 },
  ],
};
...
const TOTAL_BUDGET = 5.0; // trillion rubles (equivalent to 5000 billion)
...
export default function AnalyticsChart({ isLoading }: { isLoading?: boolean }) {
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-[#0F172A] rounded-2xl p-6 sm:p-8 border border-[#E2E8F0] dark:border-slate-800 shadow-xs flex flex-col gap-8 min-h-[400px]">
        <div className="flex flex-col lg:flex-row justify-between gap-5 border-b border-slate-100 dark:border-slate-850 pb-5">
  ...
  return (
    <div id="analytics_dashboard" className="bg-white rounded-2xl p-5 sm:p-8 border border-[#E2E8F0] shadow-xs flex flex-col gap-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 border-b border-[#F1F5F9] pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#475569] text-xs sm:text-sm leading-relaxed mt-0.5">
              Изучите структуру расходов города Москвы на 2026 год, погружайтесь в подкатегории и управляйте масштабом.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          ...
          <div className="inline-flex bg-[#F1F5F9] p-1 rounded-xl border border-[#E2E8F0] relative">
            <button
              onClick={() => setDisplayUnit('percent')}
              className={cn(
                "px-3 py-1.5 rounded-lg text-xs font-black transition-all select-none outline-none",
                displayUnit === 'percent' ? "bg-white text-[#CC1111] shadow-xs" : "text-[#475569] hover:text-[#0F172A]"
              )}
            >
              %
            </button>
            <button
              ...
              className={cn(
                "px-3 py-1.5 rounded-lg text-xs font-black transition-all select-none outline-none",
                displayUnit === 'currency' ? "bg-white text-[#CC1111] shadow-xs" : "text-[#475569] hover:text-[#0F172A]"
              )}
            >
              ₽
            </button>
          ...
          <div className="inline-flex bg-[#F1F5F9] p-1 rounded-xl border border-[#E2E8F0] relative">
            <button
              ...
              className={cn(
                ...
                !isPersonal ? "text-[#0F172A]" : "text-[#64748B] hover:text-[#0F172A]"
              )}
            >
              <Activity size={14} />
              Бюджет Москвы
              {!isPersonal && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-white rounded-lg shadow-xs ring-1 ring-black/5 -z-10"
            ...
            <button
              ...
              className={cn(
                "relative flex justify-center items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-black transition-colors duration-200 z-10 outline-none select-none",
                isPersonal ? "text-white" : "text-[#64748B] hover:text-[#0F172A]"
              )}
            >
              <Wallet size={14} />
              Мой вклад
      ...
      {isPersonal && (
        ...
        >
          <div className="p-2 bg-emerald-50 dark:bg-emerald-950/45 text-emerald-600 dark:text-emerald-450 rounded-lg shrink-0">
            <Wallet size={16} />
          </div>
          <div className="text-xs sm:text-sm">
            <h4 className="font-extrabold text-[#0F172A] dark:text-slate-100">Откуда берутся эти цифры?</h4>
            <p className="text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
              Исходя из ваших расчетных данных НДФЛ в размере <span className="font-mono text-[#0F172A] dark:text-slate-200 font-extrabold">{formatCurrency(personalTax)}</span>, ваш индивидуальный вклад деликатно распределяется пропорционально фактическим расходам Правительства Москвы. Например: ваш вклад в <span className="text-slate-800 dark:text-slate-350 font-bold">Образование (32% бюджета)</span> составляет <span className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">{formatCurrency(personalTax * 0.32)}</span>, а в <span className="text-slate-805 dark:text-slate-300 font-bold">Здравоохранение (20%)</span> составляет <span className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">{formatCurrency(personalTax * 0.20)}</span>.
            </p>
          </div>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {currentData.map((item) => {
              ...
              return (
                <motion.div 
                  ...
                  className={cn(
                    "border rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-all duration-300 cursor-pointer select-none flex flex-col border-slate-200",
                    isSelected 
                      ? "bg-slate-50 border-[#CC1111] ring-4 ring-[#CC1111]/5 sm:col-span-2" 
                      : isAnySelected 
                        ? "opacity-55 hover:opacity-100 bg-white" 
                  ...
                  <div className="flex flex-1">
                    ...
                    <div className="flex-1 p-4 flex flex-col justify-between gap-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {item.kpis.map((k, idx) => (
                          <span key={idx} className="bg-slate-100 border border-slate-200 text-[#0F172A] text-[9px] font-extrabold px-2 py-0.5 rounded-md tracking-tight">
                            {k}
                          </span>
                        ))}
                      </div>
                      ...
                      <AnimatePresence>
                        {isSelected && (
                          <motion.div 
                            ...
                            className="border-t border-slate-200 mt-3 pt-3 space-y-3"
                          >
                            ...
                            <div className="grid grid-cols-1 gap-2">
                              {SUBCATEGORIES[item.id]?.map((sub, sIdx) => {
                                ...
                                return (
                                  <div key={sIdx} className="bg-white border border-slate-200 rounded-xl p-3 flex flex-col gap-2">
                                    ...
                                    <div className="flex justify-between items-center text-[10px] text-slate-400">
                                      ...
                                      <span className="font-mono font-bold text-slate-800">{subRubleAmount}</span>
                                    </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 flex flex-col items-center justify-center bg-slate-50 rounded-2xl p-6 border border-slate-200">
              ...
              {currentData.map((s) => {
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      "bg-white p-3 rounded-xl border transition-all duration-200 cursor-pointer",
                      isSelected ? "border-[#CC1111] ring-3 ring-[#CC1111]/5" : "border-slate-200"
                    )}
                  >
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      <span className="text-xs font-black text-[#0F172A] font-mono">{progressRuble}</span>
                    </div>
      ...
      <div className="border-t border-[#F1F5F9] pt-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-base font-black text-[#0F172A] tracking-tight">Развитие административных округов г. Москвы</h3>
            </div>
          </div>
        ...
        {myDistrict && (
          <div className="bg-amber-50 rounded-xl p-3 border border-amber-200 text-[#0F172A] flex items-center justify-between gap-2.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-7 flex flex-col gap-3.5 bg-slate-50 border border-slate-200 rounded-2xl p-4.5 sm:p-5">
            <div className="flex items-center justify-between">
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isSelected 
                        ? "bg-white border-[#CC1111] shadow-xs animate-pulse-once" 
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-black text-[#0F172A]">{dist.label}</span>
                        {isHome && (
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-xs font-black text-slate-800 font-mono block leading-none">{dist.perCapita}</span>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between bg-white border border-slate-200 rounded-2xl p-5 shadow-xs relative overflow-hidden">
            ...
            <div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    myDistrict === selectedDistrict.id 
                      ...
                      : "bg-[#F8FAFC] border-slate-200 text-[#475569] hover:bg-slate-50 hover:border-slate-300"
                  )}
                >
                  {myDistrict === selectedDistrict.id ? (
                    <>
              ...
              <h3 className="text-lg font-black text-[#0F172A] tracking-tight mb-4 mt-1">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <span className="text-lg font-black font-mono text-[#0F172A] tracking-tight block">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isActive 
                        ...
                        : "bg-[#F8FAFC] border-slate-200 text-[#475569] hover:bg-slate-50"
                    )}
                  >
                    {isHome && (
