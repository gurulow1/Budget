...
export default function TaxCalculator({ activePersona, onCalculate, isCompleted, isLoading }: TaxCalculatorProps) {
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-[#0F172A] rounded-2xl p-6 sm:p-8 border border-[#E2E8F0] dark:border-slate-800 shadow-xs flex flex-col flex-1 h-auto md:h-full justify-between min-h-[350px]">
  ...
  return (
    <div className="bg-white dark:bg-[#0F172A] rounded-2xl p-6 sm:p-8 border border-[#E2E8F0] dark:border-slate-800 shadow-xs transition-shadow duration-300 flex flex-col flex-1 h-auto md:h-full relative">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <h2 className="text-lg font-bold text-[#0F172A] dark:text-slate-150 tracking-tight">
      ...
      <div className="flex flex-col gap-6 flex-1">
            ...
            {!isMobile ? (
              <div className="flex items-center gap-1.5 shrink-0 bg-[#F8FAFC] dark:bg-slate-950 border border-[#E2E8F0] dark:border-slate-800 px-2 py-1 rounded-md font-mono">
                <input
                  ...
                  className="w-20 text-right font-bold text-xs sm:text-sm text-[#0F172A] dark:text-slate-100 bg-transparent border-none outline-none focus:ring-0 p-0"
                  placeholder="0"
                />
                <span className="text-xs text-slate-400">₽</span>
              </div>
            ) : (
              <span className="text-xs font-bold text-[#0F172A] dark:text-slate-300 bg-slate-50 dark:bg-slate-900 border border-slate-150 dark:border-slate-850 px-2 py-0.5 rounded-md font-mono shrink-0">
                Лимит: 300к
              </span>
            )}
          </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-stretch justify-between gap-2.5 bg-[#F8FAFC] dark:bg-slate-950 border border-[#E2E8F0] dark:border-slate-800 p-1.5 rounded-xl">
                <button
                  ...
                  className="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-800 rounded-lg border border-[#E2E8F0] dark:border-slate-700 text-slate-500 font-bold active:bg-slate-100 active:scale-95 transition-all text-xs shadow-xs"
                >
                  -5 тыс.
                </button>
                <div className="flex-1 flex flex-col items-center justify-center min-w-0">
                  <input
                    ...
                    value={eduCost === 0 ? '' : eduCost.toLocaleString('ru-RU')}
                    onChange={(e) => handleEduInputChange(e.target.value)}
                    className="w-full text-center font-bold text-base text-[#0F172A] dark:text-slate-100 bg-transparent border-none outline-none p-0 font-mono focus:ring-0 select-text"
                    placeholder="0"
                  />
        ...
        <div className="bg-emerald-50/80 dark:bg-[#065F46]/10 border border-emerald-250 dark:border-emerald-800/40 rounded-xl p-4 flex items-center justify-between gap-3 shadow-3xs transition-all animate-fade-in">
        ...
        <div className="mt-auto pt-4 flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
          {isCompleted ? (
            <div className="flex items-center gap-3 bg-[#065f46] text-white px-5 py-4 rounded-xl border border-emerald-800 shadow-md flex-1 animate-fade-in font-bold text-xs sm:text-sm shadow-emerald-900/10">
              <span className="w-5 h-5 flex items-center justify-center rounded-full bg-white/20 shrink-0 text-xs font-black select-none">✓</span>
            ...
            {isCompleted && (
              ...
              >
                <div className="flex flex-col">
                  ...
                  <span className="text-sm font-bold tracking-tight text-emerald-700 dark:text-emerald-350">
