...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0F19] text-[#0F172A] dark:text-slate-100 flex flex-col md:py-6 select-none relative pb-16 md:pb-0">
      <div className="w-full max-w-6xl px-4 sm:px-6 lg:px-8 mx-auto flex flex-col gap-4 md:gap-6 flex-1 pb-16 md:pb-4 relative z-10">
        ...
        <main className="md:hidden flex-1 px-0.5 space-y-4 pb-36">
        ...
        <footer className="hidden md:flex mt-8 pt-6 border-t border-[#E2E8F0] text-center flex-col sm:flex-row items-center justify-between gap-4 text-xs font-semibold text-[#475569]">
          ...
          <div className="flex items-center gap-4">
            <a href="#services" className="hover:text-slate-600 transition-colors">Справочная служба</a>
            <span>•</span>
            <a href="#active-citizen" className="hover:text-[#CC1111] transition-colors text-[#475569] font-bold">Активный Гражданин</a>
          </div>
        </footer>

      </div>

      {/* MOBILE STICKY BOTTOM ACTIONS BAR (Audit #2: Height 64px (h-16), sits above the 16h mobile navigation tray) */}
      {activeMobileTab === 'calc' && (
        <div className="md:hidden fixed bottom-16 left-0 right-0 h-16 bg-white border-t border-slate-100 flex items-stretch z-40 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
          <button
            ...
            className={cn(
              ...
              calculatorTaskCompleted 
                ? "bg-slate-100 text-[#0F172A] font-extrabold" 
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#E2E8F0] z-50 h-16 flex items-center justify-around px-2 shadow-lg">
        {/* Tab 1: Calculator */}
        <button 
          onClick={() => setActiveMobileTab('calc')}
          className={cn(
            "relative flex flex-col items-center justify-center gap-1.5 flex-1 py-1 transition-all outline-none h-full",
            activeMobileTab === 'calc' ? "text-[#CC1111]" : "text-slate-400"
          )}
        >
          {activeMobileTab === 'calc' && (
            <motion.div 
        ...
        <button 
          ...
          className={cn(
            "relative flex flex-col items-center justify-center gap-1.5 flex-1 py-1 transition-all outline-none h-full",
            activeMobileTab === 'quests' ? "text-[#CC1111]" : "text-slate-400"
          )}
        >
          {activeMobileTab === 'quests' && (
            <motion.div 
          ...
          <div className="relative">
            ...
            {availableQuizzesCount > 0 && (
              <span className="absolute -top-1.5 -right-2 bg-[#CC1111] text-white text-[9px] font-black h-4 min-w-4 px-1 rounded-full flex items-center justify-center border-2 border-white animate-pulse">
        ...
        <button 
          ...
          className={cn(
            "relative flex flex-col items-center justify-center gap-1.5 flex-1 py-1 transition-all outline-none h-full",
            activeMobileTab === 'analytics' ? "text-[#CC1111]" : "text-slate-400"
          )}
        >
          {activeMobileTab === 'analytics' && (
            <motion.div 
      ...
      <BudgetAIChatDrawer activeMobileTab={activeMobileTab} />
    </div>
  );
}
